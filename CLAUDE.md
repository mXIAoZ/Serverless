...
- Run end-to-end smoke/integration test, including Node.js/Java bootstrap checks: `./test.sh`
...
- `runtime/bootstrap/python3_bootstrap.py` loads Python handlers from `/function`. `runtime/cmd/go-bootstrap/` executes a user-provided `/function/bootstrap` binary for Go functions and requires JSON stdout. `runtime/bootstrap/nodejs_bootstrap.js` loads CommonJS Node.js handlers. `runtime/bootstrap/java/JavaBootstrap.java` is built into `java-bootstrap.jar` and invokes `ClassName::methodName` reflection handlers.
...
- `runtime/cmd/runtime/main.go` selects bootstrap by `FUNCTION_RUNTIME`: `python3` uses `python3 /runtime/bootstrap/python3_bootstrap.py`, `go` uses `/runtime/bootstrap/go-bootstrap`, `nodejs` uses `node /runtime/bootstrap/nodejs_bootstrap.js`, and `java` uses `java -cp /runtime/bootstrap/java-bootstrap.jar JavaBootstrap`.
